...
export function StructuredReportComponent({ report }: StructuredReportProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <p className="text-2xl font-bold text-blue-900 mb-2">{formatPercentage(summary.overall_performance.total_return.value)}</p>
          ...
          <div className="p-4 bg-red-50 rounded-lg border border-red-200">
            ...
            <p className="text-2xl font-bold text-red-900 mb-2">{formatPercentage(summary.overall_performance.max_drawdown.value)}</p>
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold text-[#1f2937] border-b border-gray-200 pb-2">
          {summary.strategy_execution.headline}
        </h2>
        
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="p-4 bg-gray-50 rounded-lg">
            <h3 className="text-lg font-medium text-gray-800 mb-2">총 거래 횟수</h3>
            <p className="text-xl font-bold text-gray-900">{summary.strategy_execution.execution_details.total_trades === 'N/A' || !summary.strategy_execution.execution_details.total_trades ? '0' : summary.strategy_execution.execution_details.total_trades}</p>
          </div>
          
          <div className="p-4 bg-gray-50 rounded-lg">
            <h3 className="text-lg font-medium text-gray-800 mb-2">승률</h3>
            <p className="text-xl font-bold text-gray-900">{summary.strategy_execution.execution_details.win_rate}</p>
          </div>
          
          <div className="p-4 bg-gray-50 rounded-lg">
            <h3 className="text-lg font-medium text-gray-800 mb-2">손익비</h3>
            <p className="text-xl font-bold text-gray-900">{summary.strategy_execution.execution_details.profit_loss_ratio}</p>
          </div>
        </div>

        {/* Stop Loss Cases */}
        {summary.strategy_execution.stop_loss_cases.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-lg font-medium text-red-700">손절 실행 사례</h3>
            <div className="space-y-3">
              {summary.strategy_execution.stop_loss_cases.map((case_, index) => (
                <div key={index} className="p-4 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-sm font-medium text-red-800">{case_.date}</span>
                    <span className="text-sm text-red-600 bg-red-100 px-2 py-1 rounded">{case_.action}</span>
                  </div>
                  <p className="text-sm text-red-700">{case_.reason}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Take Profit Cases */}
        {summary.strategy_execution.take_profit_cases.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-lg font-medium text-green-700">익절 실행 사례</h3>
            <div className="space-y-3">
              {summary.strategy_execution.take_profit_cases.map((case_, index) => (
                <div key={index} className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-sm font-medium text-green-800">{case_.date}</span>
                    <span className="text-sm text-green-600 bg-green-100 px-2 py-1 rounded">{case_.action}</span>
                  </div>
                  <p className="text-sm text-green-700">{case_.reason}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <h3 className="text-lg font-medium text-blue-800 mb-2">효과성 분석</h3>
          <p className="text-sm text-blue-700">{summary.strategy_execution.effectiveness_analysis}</p>
        </div>
      </section>
